   ...
   void HGSW_Setup( mat_hgsw<Zq, Params, Params::n, Params::ell_pr> &S,
    ...
    auto HGSW_Add( pair<mat_hgsw<Zq, Params, Params::mq, Params::n + Params::ell_pr>, mat_hgsw<Zq, Params, Params::mq, Params::n + Params::ell_pr>> *Ct,
        ...
        for(uint32_t j = 0; j < Params::m[t] / Params::nu; j++) {
            for(uint32_t b = 0; b < Params::nu / 8; b++) {
                pair<mat_hgsw<Zq, Params, 1, Params::n + Params::ell_pr>, mat_hgsw<Zq, Params, 1, Params::n + Params::ell_pr>> batch_acc;

                for(uint32_t i = 0; i < 8; i++) {
                    uint32_t global_i = b * 8 + i;

                    mat_hgsw<zp, Params, 1, Params::w> a_arr;
                    for(uint32_t k = 0; k < Params::ell_p; k++) {
                        for(uint32_t l = 0; l < Params::f; l++) {
                            a_arr[0][k][l] = a[j * Params::nu + global_i][0][0][l];
                        }
                    }
                    ...
                    tmp.first = atT_i_rand.first * Ct[j * Params::nu + global_i].first;
                    tmp.second = atT_i_rand.second * Ct[j * Params::nu + global_i].second;

                    batch_acc.first += tmp.first;
                    batch_acc.second += tmp.second;
                }

                // Flush the batch accumulator to shrink it back to mod p
                for(uint32_t k = Params::n; k < Params::n + Params::ell_pr; k++) {
                    INTT(batch_acc.first[0][k], Params::qbar, prm::inv_n_qbar, prm::root_inv_qbar);
                    for(uint32_t l = 0; l < Params::d; l++) {
                        uint64_t val = (batch_acc.first[0][k][l] >> prm::qbar_bit) * prm::qbar_bitmask;
                        batch_acc.first[0][k][l] = ((batch_acc.first[0][k][l] - (val & prm::qbar)) + prm::q) % prm::p;
                    }
                    NTT(batch_acc.first[0][k], Params::qbar, prm::rp_conv_fact_qbar, prm::root_qbar);
                }

                ctstar.first += batch_acc.first;
                ctstar.second += batch_acc.second;
            }
        }   
        ...
        for(uint32_t j = 0; j < Params::m[t] / Params::nu; j++) {
            ...
            for(uint32_t i = Params::n; i < Params::n + Params::ell_pr; i++) {  
                for (uint32_t k = 0; k < Params::d; k++) {
                    zero_n_yT[0][i][k] = (dgg.GenerateIntegerKarney(0, Params::sig_ry) + Params::q) % Params::q; // Appx.F Alg.3 Line 12, use sig_ry instead of sig_r
                }
            }
    ...
    auto HGSW_Decrypt( mat_hgsw<Zq, Params, Params::n, Params::ell_pr> &S,
        ...
        for (uint32_t i = 0; i < Params::n + Params::ell_pr; i++) {
            for (uint32_t j = 0; j < Params::ell_pr; j++) {
                for (uint32_t l = 0; l < Params::d; l++) {
                    uint64_t sign = (S_barT[i][j][l] >> Params::q_bit) * Params::q_bitmask; // adjust SbarT mod q' 
